...
export class BookingPanelComponent implements OnInit {
  ...
  displayedColumns = ['id','FIO','telNumber','startDate','endDate','sumPrice'];
  ...
  constructor(
    ...
    private sanService:SanService
      ...
      this.selectedSan = this.router.getCurrentNavigation().extras.state['selectedSan']
      this.endDate = new Date();
      this.startDate = new Date(this.endDate.getFullYear(), this.endDate.getMonth()-1,this.endDate.getDay());
  ...
  public getAll(){
   ...
   this.sanId = this.route.snapshot.paramMap.get('id');   
    let resp = this.service.getAll({startDate:this.startDate.toISOString().slice(0,10),endDate:this.endDate.toISOString().slice(0,10),
                                    telNumber:this.telNumber,page:0,size:10,sanId: this.sanId?this.sanId:undefined,status:this.status,minPrice:this.minPrice,maxPrice:this.maxPrice})
    resp.subscribe(res=>{
 ...
 public incrementPage(){
  ...
  public setSanId(san:Sanatory){
    ...
    this.router.navigate(['/main/booking',String(san.id)],{state:{selectedSan:this.selectedSan}});
    this.getAll();
  }
...
export class RoomForBookingComponent implements OnInit {
 ...
 public getRoomClassName(id:number){
   if(id)
   return this.roomClasses.find(r=>r.id === id).name;
   return ""
  }
